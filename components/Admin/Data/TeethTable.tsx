...
const TeethMaterialsTable = () => {
  ...
  const columns = useMemo<Column<FullTeethMaterial>[]>(
    () => [
      ...
      },
      {
        ...
        accessor: 'id',
        Cell: ({ value }) => (
          <Link href={`/admin/edit/material?id=${value}`}>
            <Button colorScheme='red' leftIcon={<Icon as={FaEdit} />} size='sm' variant='outline'>
              Edit
            </Button>
          </Link>
        ),
  ...
  return _.isArray(materialsData) ? (
    <PaginatedTable<FullTeethMaterial> colorScheme='red' columns={columns} data={materialsData} />
  ) : (
    <div />
  );
};
